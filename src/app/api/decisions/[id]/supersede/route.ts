...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!oldDecision) return NextResponse.json({ error: API_ERROR.notFound }, { status: 404 });
    ...
    if (oldDecision.status !== "ACTIVE") return NextResponse.json({ error: API_ERROR.onlyActiveSupersedes }, { status: 409 });
    ...
    if (branchIds.some((branchId) => !branches.some((branch) => branch.id === branchId))) return NextResponse.json({ error: API_ERROR.branchOutsideContext }, { status: 403 });
