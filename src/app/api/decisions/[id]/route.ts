...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return decision ? NextResponse.json({ decision: { ...decision, effectiveStatus: effectiveDecisionState(decision.status, decision.validUntil) } }) : NextResponse.json({ error: API_ERROR.notFound }, { status: 404 });
  } catch (error) {
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!existing) return NextResponse.json({ error: API_ERROR.notFound }, { status: 404 });
    ...
    if (!existing.affectedBranches.some((link) => administrable.some((branch) => branch.id === link.branchId))) return NextResponse.json({ error: API_ERROR.notFound }, { status: 404 });
    const body = updateSchema.parse(await request.json());
    if (body.validUntil && body.validUntil <= existing.validFrom) return NextResponse.json({ error: API_ERROR.validUntilBeforeFrom }, { status: 422 });
